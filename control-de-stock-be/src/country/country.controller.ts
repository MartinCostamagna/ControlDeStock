...
export class CountriesController {
  ...
  create(
    ...
    this.logger.log(
      `Recibida solicitud para crear país: ${JSON.stringify(createCountryDto)}`,
    );
    return this.countriesService.create(
      createCountryDto,
      false,
    ) as Promise<CountryResponseDto>;
  }
  ...
  findAll(
    ...
    this.logger.log(
      `Recibida solicitud para obtener todos los países con paginación: ${JSON.stringify(paginationDto)}`,
    );
  ...
  findOne(@Param('id', ParseIntPipe) id: number): Promise<CountryResponseDto> {
    ...
    return this.countriesService.findOne(
      id,
      false,
    ) as Promise<CountryResponseDto>;
  }
