...
export class ProvincesController {
  ...
  create(
    ...
    this.logger.log(
      `Recibida solicitud para crear provincia: ${JSON.stringify(createProvinceDto)}`,
    );
    return this.provincesService.create(
      createProvinceDto,
      false,
    ) as Promise<ProvinceResponseDto>;
  }
  ...
  findAll(
    ...
    this.logger.log(
      `Recibida solicitud para obtener todas las provincias con paginación: ${JSON.stringify(paginationDto)}`,
    );
  ...
  async searchByName(
    ...
    this.logger.log(
      `Buscando provincias por nombre: ${name} con paginación: ${JSON.stringify(paginationDto)}`,
    );
  ...
  findOne(@Param('id', ParseIntPipe) id: number): Promise<ProvinceResponseDto> {
    ...
    return this.provincesService.findOne(
      id,
      false,
    ) as Promise<ProvinceResponseDto>;
  }
  ...
  findProvincesByCountry(@Param('countryId', ParseIntPipe) countryId: number) {
    this.logger.log(
      `Recibida solicitud para obtener provincias del país con ID: ${countryId}`,
    );
  ...
  updatePut(
    ...
    this.logger.log(
      `Recibida solicitud PUT para reemplazar provincia ID: ${id}`,
    );
  ...
  updatePatch(
    ...
    this.logger.log(
      `Recibida solicitud PATCH para actualizar provincia ID: ${id}`,
    );
